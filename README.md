...
The v0.3 loader accepts the complete official v0.2.5 config by removing only
...
NuSelf schema through a lock-aware `mode=ro` connection that participates in
normal WAL coordination. Ordinary startup checks only schema identity, not the
full-database `quick_check`; redirected parents fail before external effects,
while empty or unrelated databases, incomplete schemas, corruption encountered
while reading identity metadata, and unsupported future schemas fail closed
before schema initialization or business writes. SQLite may access its WAL/SHM
...
The current stable package is `0.3.0`.
