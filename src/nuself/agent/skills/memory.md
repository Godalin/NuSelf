...
Use {tool:search} before answering unless the answer is fully present in the current visible conversation or already provided in `Relevant memory context`. If the result is empty, say that no matching stored memory was found; do not imply NuSelf has no memory at all.
