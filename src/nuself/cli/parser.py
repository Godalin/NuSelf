...
class InteractiveHandlers:
    default_entrypoint: Handler
    chat: Handler
    attach: Handler
    open_thread: Handler
    reason_watch: Handler
...
def _add_log_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--component", choices=list(LOG_COMPONENTS), default=None
    )
...
def build_parser(handlers: InteractiveHandlers) -> argparse.ArgumentParser:
    ...
    _add_handler(parser, handlers.default_entrypoint)
    ...
    _add_handler(daemon_attach_parser, handlers.attach)
    ...
    _add_handler(chat_parser, handlers.chat)
    ...
    _add_handler(attach_parser, handlers.attach)
    ...
    _add_handler(thread_open_parser, handlers.open_thread)
    ...
    _add_handler(reason_watch_parser, handlers.reason_watch)
