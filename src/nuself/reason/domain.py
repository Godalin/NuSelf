...
def _legacy_tool_call_to_log(item: list[object]) -> dict[str, object]:
    ...
    service: str = ""
    if len(item) >= 4 and isinstance(item[1], str) and isinstance(item[2], dict):
        service = item[1]
        args = cast(dict[str, object], item[2])
        result = str(item[3]) if item[3] is not None else None
    elif len(item) >= 3 and isinstance(item[1], dict):
        args = cast(dict[str, object], item[1])
        result = str(item[2]) if item[2] is not None else None
