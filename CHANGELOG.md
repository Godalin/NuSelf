...
- Added `reasoning_prompt_gen` StructuredTool — any agent can call it to generate a topic-specific reasoning system prompt via another LLM.
- Each reasoning thread now uses a topic-specific system prompt (generated by an LLM at thread creation) instead of the generic hardcoded prompt. The prompt is shown in `:reason show` and `:reason watch` under `reasoning_prompt:`.
- `:reason show` and `:reason watch` header now always displays all global thread fields including `mandates`, `reasoning_prompt`, `description`, and `evidence` — empty fields show `(none)` instead of being hidden.

### Fixed

- `:reason advance` no longer silently falls back to a raw `ChatLLM.complete()` path without tools or structured output. All advance paths (CLI, TUI, scheduler) now use the same LangGraph `create_agent` graph with tool support and `ReasonStepOutput` structured output. If no LangChain models are configured, advance raises `RuntimeError` instead of silently degrading.
- `:reason watch` and `:reason show` now correctly render tool logs from legacy step files. Tool results in `step.tool_logs` are no longer truncated to 200 characters by the middleware capture layer.
- `reason_propose` tool now requires `mandates` and `active_items` parameters — the chat agent must provide them when proposing a thread, ensuring every thread starts with explicit rules and tracked items.
- Step output field is now required in step validation — missing `output` causes the step to be rejected.
