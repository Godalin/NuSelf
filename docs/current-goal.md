...
Remove the orphaned daemon request-ID generator API. Request identity remains a
fresh UUID hex value generated by the `DaemonRequest` field factory; no caller,
test, or extension boundary consumes a standalone helper.

## Ordered Steps

1. Confirm request-ID creation has no consumer beyond the dataclass default.
2. Inline fresh UUID-hex generation into `DaemonRequest.request_id` and remove
   `new_request_id()`.
3. Preserve explicit request IDs during decoding and caller construction.
4. Run protocol/daemon tests, Pyright, full pytest, and package build; update
   evidence and commit without pushing.

## Exclusions

- Do not change the request-ID representation or generation timing.
- Do not regenerate IDs during decoding or retries.
- Do not change JSONL protocol validation, correlation, or audit behavior.
