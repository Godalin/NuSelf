...
success payload. The active CLI/REPL frontend asks through its
`TerminalApprovalPort`, then retries the same message, conversation, and
`turn_id` with an `ApprovalDecision` bound to that exact request. The daemon
accepts the decision only when component, operation, action, resource, risk,
and summary all match the request generated by the retry. A changed request
produces a new challenge; it never consumes a stale decision. A declined
decision reaches the Tool adapter as a normal no-write Tool result so the Agent
can respond naturally. Missing approval infrastructure is not represented as a
user decline, and activity events remain non-executable projections.
