...
Status: CURRENT — general-purpose tracked items with chat tools, `topic` field, and mandates.
...
| `start_thread`           | `(topic, working_summary?, evidence_refs?, source_trace_ids?, priority?, active_items?, mandates?)` |
...
When an explicit `step` is provided to `advance_thread`, the service uses it instead of a placeholder. The `ReasonAdvancer` generates the step:
...
First-pass context retrieval scope: thread's own `working_summary`, `active_items`, `pending_items`, `mandates`, and `evidence_refs`. No external retrieval from memory/reflection/trace in first implementation.
...
- `active_items` (optional) — initial tracked items, each with `"label"` (required), `"description"` (optional), `"kind"` (optional free-text tag).
- `mandates` (optional) — required actions the advancer MUST follow on every advance.
...
7. **Evidence refs section**: `evidence_refs:`, each rendered as markdown.
...
`--full` renders all step fields even when empty, using `(no ...)` placeholders instead of omitting sections.
...
- First-pass context retrieval: thread-local only (working_summary, active_items, pending_items, next_steps, mandates, evidence_refs).
