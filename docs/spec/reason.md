...
If prompt generation is unavailable, the thread may fall back to the generic reason advance prompt, but the intended v0.2.0 path is: create thread -> generate and persist `reasoning_prompt` -> use that prompt for every advance of that thread.
...
| `start_thread`           | `(topic, working_summary?, evidence_refs?, source_trace_ids?, priority?, active_items?, mandates?)`; generates `reasoning_prompt` when a LangChain model is configured |
...
- Uses the thread's generated `reasoning_prompt` as system prompt when present; otherwise uses the generic reason advance prompt. The generated prompt is produced by a separate prompt-generation agent/tool, not by the advancer itself.
