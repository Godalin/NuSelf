...
Segments are the unit of incremental work for the export scheduler handler.
...
- Missing, unreadable, non-object, or schema-invalid manifests are corrupt job
  state. The scheduler handler logs `export_job_manifest_invalid` (or the
  reconciliation equivalent), records degraded scheduler health, and does not compose or
  automatically retry that job.
- `complete` and `failed` manifests are terminal and are never recomposed from
  a stale task wake-up.
...
**Rationale**: The `manifest.json` in the job directory is the real persistent
state. The scheduler task is only a signal — "there is a pending job, go look
at its manifest". Persisting that signal separately is unnecessary I/O with
duplicate, partial-write, and stale-claim failure modes. The scheduler therefore
needs no export-specific `queue/`, `processing/`, or `failed/` directory tree.

#### Job wake-up

A wake-up is an immutable `JobMessage` backed entirely by a versioned
...
The chat tool receives a typed `JobSink` through constructor injection; the
reason module does not install a process-global enqueue callback.

The export service seals one definition for `reason.output.export` during
daemon composition. Allowed producers are `reasoning`, `daemon_retry`, and
...
and invalid values are programming errors; they are never admitted and therefore
do not produce an `export_job_type_ignored` audit.

The scheduler identity includes the job, thread, and retry attempt, so duplicate
wake-ups for the same attempt coalesce. Its `reason:{thread_id}` resource lane
serializes export composition with other work for that Reason thread. Capacity
pressure never blocks the caller or discards the durable manifest; an admission
failure is diagnosed, and startup recovery can rediscover the incomplete job.

The scheduler handler reconstructs the job data path from `thread_id` and `job_id`:
`<authority-root>/exports/reason/{thread_id}/jobs/{job_id}/manifest.json`.
...
- On failure, the export service checks
  `manifest.attempts < MAX_EXPORT_ATTEMPTS`.
- If retryable, it schedules the job identity with exponential backoff (capped at 600s).
- When the delay expires, the scheduler executes a new typed wake-up for the
  same durable job.
- If `attempts >= MAX_EXPORT_ATTEMPTS`, the service updates the manifest status
  to `failed` and does not submit another wake-up.
- A failed compose attempt is retryable only after its incremented attempt
  ...
  manifest.
- If the manifest cannot be decoded, processing fails closed. If the retry-state
  write fails, the service logs `export_job_state_persist_failed` and requests
  one delayed reconciliation wake-up without pretending the attempt was
  durably recorded.
- Once retry state is durably persisted, audit storage failure cannot suppress
  ...
  `export_retry_schedule_failed` and requests one delayed reconciliation
  wake-up. If scheduler admission itself is unavailable, the durable manifest
  remains recoverable on daemon restart.
...
This restores pending work across daemon restarts without a persistent
export-specific queue. The scan cost is bounded by durable job directories,
while in-memory admission uses the scheduler's shared bound.
...
The first chat-facing export tool call must be approval-gated, but the agent
should call it directly when the user asks for an export rather than waiting
for a separate confirmation turn. During the call, it prompts the user for
confirmation, then plans the job, writes the manifest, submits a typed wake-up
to the unified scheduler, and returns structured JSON that includes whether
the user approved and, when approved, the job metadata. The scheduler handler
composes chunks and writes the final artifact; daemon startup first reconciles
incomplete manifests into scheduler tasks.

### Daemon service ownership

`nuself.reason.export.ReasonExportService` owns reason-export domain
...
All export handler lifecycle and caught-failure audit writes use the shared
observable best-effort boundary. Audit failure cannot change an already-made
task, manifest, retry, composition, reconciliation, or shutdown decision.
...
and export-handler operations. `reasoning` records describe durable output
artifacts; `daemon` records describe wake-up delivery, task consumption,
