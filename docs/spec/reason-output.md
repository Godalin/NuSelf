...
The worker reconstructs the job data path from `thread_id` and `job_id`:
...
1. **Re-enqueue incomplete jobs**: Scan `<authority-root>/exports/reason/*/jobs/*/manifest.json`. For each manifest with status other than `complete` or `failed`, construct a typed `JobMessage` and push it into the in-memory queue. This recovers any jobs that were in flight when the daemon last exited.
